...
public class TaskTest {
	...
	void test(String intLine, String strLine, String expectedLine) {
		List<Integer> integerList = Arrays.stream(intLine.split("~"))
			.map(Integer::parseInt)
			.collect(Collectors.toList());

		List<String> stringList = Arrays.stream(strLine.split("~"))
			.collect(Collectors.toList());

		List<String> actual = Task.f(integerList, stringList);

		List<String> expected = Arrays.stream(expectedLine.split("~"))
			.collect(Collectors.toList());
