...
public class TaskTest {
	...
	void test(String nameLine, String yearsLine, String expectedLine) {
		List<Entrant> nameList = Arrays.stream(nameLine.split("\\s+"))
			.map(String::trim)
			.map(s -> s.split("~"))
			.map(br -> new Entrant(
		...
		List<Integer> yearsList = Arrays.stream(yearsLine.split("~"))
			.map(String::trim)
			.map(Integer::parseInt)
			.collect(Collectors.toList());

		List<YearSchoolStat> expected = Arrays.stream(expectedLine.split("\\s+"))
			.map(String::trim)
			.map(s -> s.split("~"))
		...
		List<YearSchoolStat> actual = Task.f(nameList, yearsList);
